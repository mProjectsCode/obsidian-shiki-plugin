...
export default class EditableCodeblockInOb extends EditableCodeblock {
	...
	override enableTabEmitIndent(el: HTMLElement, cb_tab?: (ev: KeyboardEvent)=>void, cb_up?: (ev: KeyboardEvent)=>void, cb_down?: (ev: KeyboardEvent)=>void): void {
		if (!(el instanceof HTMLInputElement || el instanceof HTMLTextAreaElement || el.isContentEditable)) return

		super.enableTabEmitIndent(el, cb_tab, cb_up, cb_down) // `tab`
	...
	override async saveContent_safe(isUpdateLanguage: boolean = true, isUpdateSource: boolean = true): Promise<void> {
		/**
	...
	override async saveContent(isUpdateLanguage: boolean = true, isUpdateSource: boolean = true): Promise<void> {
		// range
		const sectionInfo = this.ctx.getSectionInfo(this.el);
		if (!sectionInfo) {
